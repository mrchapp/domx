...
void convert_nv12_to_yuyv(unsigned char *nv12, unsigned short *yuyv, int w, int h, int s)
{
...
{
    char *dev = viddev ? viddev->path : VIDEO_DEVICE1;
    v4l2_dev_display = open(dev, O_RDWR);
    if (NOT_S(v4l2_dev_display,>=,0)) return (1);
    P("opened %s", dev);
    ...
    if (NOT_S(ioctl(v4l2_dev_display, VIDIOC_QUERYCAP, &capability),==,0))
        return (1);
    ...
    if (NOT_S(ioctl(v4l2_dev_display, VIDIOC_G_FMT, &format),==,0)) return (1);
    ...
    if (pixfmt == V4L2_PIX_FMT_NV12)
    {
        ...
        P("overriding format: %c%c%c%c", FOUR_CHARS(format.fmt.pix.pixelformat));
    }
#endif

    if (NOT_S(ioctl(v4l2_dev_display, VIDIOC_S_FMT, &format),==,0)) return (1);
    ...
    if (NOT_S(ioctl(v4l2_dev_display, VIDIOC_REQBUFS, &reqbuf),==,0)) return (1);
    ...
    for (i = 0; i < v4l2_num_buffers; i++)
    {
        ...
        if (NOT_S(ioctl(v4l2_dev_display, VIDIOC_QUERYBUF, &buffer),==,0)) return (1);

        P("%d: buffer.length=%d, buffer.m.offset=%d", i, buffer.length, buffer.m.offset);
        ...
        v4l2_display_buffers[i].start = mmap(NULL, buffer.length, PROT_READ|
                                        PROT_WRITE, MAP_SHARED,
                                        v4l2_dev_display, buffer.m.offset);
        if (NOT_S(v4l2_display_buffers[i].start,!=,MAP_FAILED)) return (1);

        P("%d: buffer.length=%d, buffer.start=%p", i, v4l2_display_buffers[i].length,
        ...
        if (pixfmt == V4L2_PIX_FMT_NV12)
        {
            ...
            v4l2_display_buffers[i].workaround = v4l2_display_buffers[i].start;
            ...
            v4l2_display_buffers[i].start = MemMgr_Alloc(blocks, 2);
            if (NOT_S(v4l2_display_buffers[i].start,!=,0)) return (1);
            P("%d: workaround.start=%d", i, v4l2_display_buffers[i].start);
            v4l2_width = width;
            v4l2_height = height;
...
void *v4l2_get_displayed_buffer(int timeout_ms)
{
    ...
    if (v4l2_bufs_in_queue > 3)
    {
        ...
        if (NOT_S(result,>,0)) return NULL;

        if (NOT_S(ioctl(v4l2_dev_display, VIDIOC_DQBUF, &filledbuffer),==,0))
            return NULL;
...
void *v4l2_display_buffer(void *pBuffer)
{
    ...
    if (NOT_I(i,<,v4l2_num_buffers)) return pBuffer;

#ifdef NV12_WORKAROUND
    if (v4l2_display_buffers[i].workaround)
    {
        convert_nv12_to_yuyv(pBuffer, v4l2_display_buffers[i].workaround,
                             v4l2_width, v4l2_height, v4l2_nv12_stride);
    }
#endif
    ...
    if (!v4l2_streaming)
    {
        if (NOT_S(ioctl(v4l2_dev_display, VIDIOC_STREAMON, &v4l2_type),==,0))
            return pBuffer;
        v4l2_streaming = 1;
    }
#endif
    ...
    else
    {
        ...
        float fps = (time1.tv_sec - time0.tv_sec) + 0.000001 * (time1.tv_usec - time0.tv_usec);
        fps /= v4l2_bufs_streamed - 8;
        if (fps) fps = 1 / fps;
        P("Q[%d] fps = %4.2g", filledbuffer.index, fps);
    }
...
int v4l2_display_done()
{
    ...
    while (v4l2_bufs_in_queue > 1)
    {
        ...
        ERR_ADD_S(res, ioctl(v4l2_dev_display, VIDIOC_DQBUF, &filledbuffer));
        // P("DQ[%d]", filledbuffer.index);
        v4l2_bufs_in_queue --;
    }
    ...
    for (i = 0; i < v4l2_num_buffers; i++)
    {
        ...
        if (v4l2_display_buffers[i].start)
            munmap(v4l2_display_buffers[i].start, v4l2_display_buffers[i].length);
    }

    if (v4l2_streaming)
    {
        ERR_ADD_S(res, ioctl(v4l2_dev_display, VIDIOC_STREAMOFF, &v4l2_type));
    }
...
OMX_BUFFERHEADERTYPE *omx_v4l2_display_buffer(OMX_BUFFERHEADERTYPE* pBufferOut)
{
    ...
    int i;
    for (i = 0; i < v4l2_num_buffers && v4l2_display_buffers[i].start != pBuffer; i++);
    if (NOT_I(i,<,v4l2_num_buffers)) return NULL;
