...
OMX_ERRORTYPE MPEG4ENC_FillData(MPEG4E_ILClient* pAppData,OMX_BUFFERHEADERTYPE *pBuf);
...
OMX_ERRORTYPE MPEG4ENC_SetParamsFromInitialDynamicParams(MPEG4E_ILClient* pAppData);
...
OMX_ERRORTYPE MPEG4E_Populate_BitFieldSettings(OMX_U32 BitFileds,OMX_U8* Count,OMX_U8* Array);
...
/*Call Back Functions*/
OMX_ERRORTYPE MPEG4ENC_EventHandler(OMX_HANDLETYPE hComponent,OMX_PTR ptrAppData,OMX_EVENTTYPE eEvent,
                           OMX_U32 nData1, OMX_U32 nData2, OMX_PTR pEventData);

OMX_ERRORTYPE MPEG4ENC_FillBufferDone (OMX_HANDLETYPE hComponent, OMX_PTR ptrAppData,
    OMX_BUFFERHEADERTYPE* pBuffer);

OMX_ERRORTYPE MPEG4ENC_EmptyBufferDone(OMX_HANDLETYPE hComponent, OMX_PTR ptrAppData,
    OMX_BUFFERHEADERTYPE* pBuffer);
...
/*Tiler Buffer <-> 1D*/
OMX_ERRORTYPE OMXMPEG4_Util_Memcpy_1Dto2D(OMX_PTR pDst2D, OMX_PTR pSrc1D, OMX_U32 nSize1D, OMX_U32 nHeight2D, OMX_U32 nWidth2D, OMX_U32 nStride2D);
OMX_ERRORTYPE OMXMPEG4_Util_Memcpy_2Dto1D(OMX_PTR pDst1D, OMX_PTR pSrc2D, OMX_U32 nSize1D, OMX_U32 nHeight2D, OMX_U32 nWidth2D, OMX_U32 nStride2D);
...
OMX_ERRORTYPE OMXMPEG4Enc_LessthanMinBufferRequirementsNum(MPEG4E_ILClient* pApplicationData);
OMX_ERRORTYPE OMXMPEG4Enc_SizeLessthanMinBufferRequirements(MPEG4E_ILClient* pApplicationData);
		...
		if(tPortDef.nPortIndex==MPEG4_APP_INPUTPORT){			
...
#else
					pTmpBuffer= TIMM_OSAL_Malloc(((pAppData->MPEG4_TestCaseParams->width*pAppData->MPEG4_TestCaseParams->height*3/2)), TIMM_OSAL_TRUE, 32 ,TIMMOSAL_MEM_SEGMENT_EXT);
#endif
			...
			for (j= 0; j < tPortDef.nBufferCountActual; j++)
			{
				if(pAppData->MPEG4_TestCaseParams->bOutAllocatebuffer){
					...
					/*USE Buffer calls*/
#ifdef OMX_MPEG4E_BUF_HEAP
        				pTmpBuffer = HeapBuf_alloc(heapHandle, (pAppData->MPEG4_TestCaseParams->width*pAppData->MPEG4_TestCaseParams->height),0);

#elif defined (OMX_MPEG4E_LINUX_TILERTEST)
					MemReqDescTiler[0].pixelFormat=PIXEL_FMT_PAGE;
					MemReqDescTiler[0].dim.len=pAppData->MPEG4_TestCaseParams->width*pAppData->MPEG4_TestCaseParams->height;
	...
	/*Tiler Interface code*/
	 ...
	 if((!feof(pAppData->fIn))&&((nRead = fread(pTmpBuffer,sizeof(OMX_U8),framesizetoread, pAppData->fIn))!=0)&&((MPEG4ClientStopFrameNum-1)>nFramesRead)){
			...
			/*frame data< the required so need not send for processing send an EOS flag with no data */
		{
		   ...
		   pTmpBuffer += (pAppData->MPEG4_TestCaseParams->height * pAppData->MPEG4_TestCaseParams->width);
...
OMX_ERRORTYPE MPEG4E_Populate_BitFieldSettings(OMX_U32 BitFields,OMX_U8* Count,OMX_U8* Array){
	OMX_ERRORTYPE eError = OMX_ErrorNone;
...
OMX_ERRORTYPE MPEG4ENC_SetParamsFromInitialDynamicParams(MPEG4E_ILClient* pApplicationData){
	OMX_ERRORTYPE eError = OMX_ErrorNone;
	...
	eError= MPEG4E_Populate_BitFieldSettings(pAppData->MPEG4_TestCaseParams->nBitEnableDynamic,&DynamicSettingsCount,DynamicSettingsArray);
		...
		switch(DynamicSettingsArray[i]){
			case 0:
				/*Frame rate*/
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynFrameRate.nFrameNumber[DynFrameCountArray_Index[i]]==0){
					bFramerate=OMX_TRUE;
			...
			case 2:
				...
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nFrameNumber[DynFrameCountArray_Index[i]]==0){					
					...
					tMESearchrange.eMVAccuracy=pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nMVAccuracy[DynFrameCountArray_Index[i]];
					tMESearchrange.nHorSearchRangeP=pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeP[DynFrameCountArray_Index[i]];
					tMESearchrange.nVerSearchRangeP=pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeP[DynFrameCountArray_Index[i]];
					tMESearchrange.nHorSearchRangeB=pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeB[DynFrameCountArray_Index[i]];
					tMESearchrange.nVerSearchRangeB=pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeB[DynFrameCountArray_Index[i]];
			...
			case 3:
				...
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.nFrameNumber[DynFrameCountArray_Index[i]]==0){					
					...
					tForceFrame.IntraRefreshVOP=pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.ForceIFrame[DynFrameCountArray_Index[i]];
			...
			case 4:
				...
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nFrameNumber[DynFrameCountArray_Index[i]]==0){					
					...
					tQPSettings.nQpI=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpI[DynFrameCountArray_Index[i]];
					tQPSettings.nQpMaxI=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxI[DynFrameCountArray_Index[i]];
					tQPSettings.nQpMinI=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinI[DynFrameCountArray_Index[i]];
					tQPSettings.nQpP=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpP[DynFrameCountArray_Index[i]];
					tQPSettings.nQpMaxI=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxP[DynFrameCountArray_Index[i]];
					tQPSettings.nQpMinI=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinP[DynFrameCountArray_Index[i]];
					tQPSettings.nQpOffsetB=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpOffsetB[DynFrameCountArray_Index[i]];
					tQPSettings.nQpMaxB=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxB[DynFrameCountArray_Index[i]];
					tQPSettings.nQpMinB=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinB[DynFrameCountArray_Index[i]];
	...
	for(i=0; i < tPortParams.nPorts ; i++){
		...
		if(tPortDef.eDir == OMX_DirInput){
			...
			if(pAppData->MPEG4_TestCaseParams->TestCaseId==6){
				...
				if(pAppData->MPEG4_TestCaseParams->nNumInputBuf>=pAppData->MPEG4_TestCaseParams->maxInterFrameInterval){
					tPortDef.nBufferCountActual=pAppData->MPEG4_TestCaseParams->nNumInputBuf;
				}else{
		...
		if(tPortDef.eDir == OMX_DirOutput){
			...
			if(pAppData->MPEG4_TestCaseParams->TestCaseId==6){
				...
				if(pAppData->MPEG4_TestCaseParams->maxInterFrameInterval>1){
					/*check for the valid Output buffer count that the user configured with*/
					if(pAppData->MPEG4_TestCaseParams->nNumOutputBuf>=1){
		...
		tAVCParams.nBFrames = pAppData->MPEG4_TestCaseParams->maxInterFrameInterval-1;
		tAVCParams.bEnableFMO= pAppData->MPEG4_TestCaseParams->bFMO;
		tAVCParams.bEntropyCodingCABAC=pAppData->MPEG4_TestCaseParams->bCABAC;
	...
	GOTO_EXIT_IF((eError != OMX_ErrorNone),eError);
	tVidEncBitRate.eControlRate =pAppData->MPEG4_TestCaseParams->RateControlAlg;
	if(bBitrate){
                           ...
                           OMX_U32 nData1, OMX_U32 nData2, OMX_PTR pEventData)
{
	...
	switch (eEvent)
	{
		case OMX_EventCmdComplete:
			retval = TIMM_OSAL_EventSet (MPEG4VE_CmdEvent, MPEG4_STATETRANSITION_COMPLETE, TIMM_OSAL_EVENT_OR);
			GOTO_EXIT_IF((retval != TIMM_OSAL_ERR_NONE), OMX_ErrorBadParameter);
			break;

		case OMX_EventError:
			MPEG4CLIENT_ERROR_PRINT( "received from the component in EventHandler=%d",(OMX_ERRORTYPE)nData1);
			retval = TIMM_OSAL_EventSet (MPEG4VE_Events, MPEG4_ERROR_EVENT, TIMM_OSAL_EVENT_OR);
			GOTO_EXIT_IF((retval != TIMM_OSAL_ERR_NONE), OMX_ErrorBadParameter);
			retval = TIMM_OSAL_EventSet (MPEG4VE_CmdEvent, MPEG4_ERROR_EVENT, TIMM_OSAL_EVENT_OR);
			GOTO_EXIT_IF((retval != TIMM_OSAL_ERR_NONE), OMX_ErrorBadParameter);
			break;
    ...
    OMX_BUFFERHEADERTYPE* pBuffer)
{
	...
	MPEG4CLIENT_ENTER_PRINT();
		...
		MPEG4CLIENT_TRACE_PRINT( "\nthe Timestamp received =%ld",pBuffer->nTimeStamp);
		i=pAppData->nFillBufferDoneCount;
	    	pAppData->TimeStampsArray[i]=pBuffer->nTimeStamp;
		MPEG4CLIENT_TRACE_PRINT( "\ncount val= %d the Timestamp saved in the array[%d]= %d",i, pAppData->nFillBufferDoneCount,pAppData->TimeStampsArray[i]);
	    	pAppData->nFillBufferDoneCount++;
		MPEG4CLIENT_TRACE_PRINT( "\ncount val after increment =%d",pAppData->nFillBufferDoneCount);
        }
	#endif
#ifdef OMX_MPEG4E_SRCHANGES
...
#endif
{
	...
	pAppData.MPEG4_TestCaseParams=(MPEG4E_TestCaseParams*)TIMM_OSAL_Malloc(sizeof(MPEG4E_TestCaseParams), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
	GOTO_EXIT_IF((pAppData.MPEG4_TestCaseParams== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
	/*allocate memory for pAppData.MPEG4_TestCaseParamsDynamic*/
	pAppData.MPEG4_TestCaseParamsDynamic =NULL;
	pAppData.MPEG4_TestCaseParamsDynamic=(MPEG4E_TestCaseParamsDynamic*)TIMM_OSAL_Malloc(sizeof(MPEG4E_TestCaseParamsDynamic), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
	GOTO_EXIT_IF((pAppData.MPEG4_TestCaseParamsDynamic== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
  ...
  pAppData.MPEG4_TestCaseParams->TestCaseId = 0;
		...
		switch(pAppData.MPEG4_TestCaseParams->TestCaseId){
			...
			MPEG4CLIENT_TRACE_PRINT( "\nRunning TestCaseId=%d",pAppData.MPEG4_TestCaseParams->TestCaseId);
				...
				MPEG4CLIENT_INFO_PRINT( "\nbefore Test case Start: Value from GetMemCounter = %d", mem_count_start);
				#ifndef MPEG4_LINUX_CLIENT
				...
				#endif
				MPEG4CLIENT_INFO_PRINT( "\n After Test case Run: Value from GetMemCounter = %d", mem_count_end);
				#ifndef MPEG4_LINUX_CLIENT
				MPEG4CLIENT_INFO_PRINT( "\n After Test case Run: Value from GetMemUsage = %d", mem_size_end);
	...
	while ((eError == OMX_ErrorNone) && ((pAppData->eState == OMX_StateExecuting) || (pAppData->eState == OMX_StatePause)) ) {
		...
		if ((pRetrievedEvents & MPEG4_INPUT_READY)) {
			...
			if((MPEG4ClientStopFrameNum>=FrameNumToProcess)){
				...
				while((numRemaining)&&((MPEG4ClientStopFrameNum>=FrameNumToProcess))){
					/*read from the pipe*/
					...
					HeapBuf_free(heapHandle, pBufferIn->pBuffer, (pAppData->MPEG4_TestCaseParams->width*pAppData->MPEG4_TestCaseParams->height*3/2));
					pBufferIn->pBuffer = HeapBuf_alloc(heapHandle, (pAppData->MPEG4_TestCaseParams->width*pAppData->MPEG4_TestCaseParams->height*3/2),0);
					...
					TilerAddr=MemMgr_Alloc(MemReqDescTiler,2);
					MPEG4CLIENT_TRACE_PRINT("\nTiler buffer allocated is %x\n",TilerAddr);
                    		pBufferIn->pBuffer = (OMX_U8 *)TilerAddr;
	...
	for(i=0;i<DynamicSettingsCount;i++){
		switch(DynamicSettingsArray[i]){
			...
			case 1:
				...
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynBitRate.nFrameNumber[DynFrameCountArray_Index[i]]==FrameNumber){
					/*Set Port Index*/
			...
			case 2:
				/*set config to OMX_TI_VIDEO_CONFIG_ME_SEARCHRANGE*/
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nFrameNumber[DynFrameCountArray_Index[i]]==FrameNumber){
					/*Set Port Index*/
					...
					MPEG4EConfigStructures->tMESearchrange.nHorSearchRangeB=pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeB[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tMESearchrange.nVerSearchRangeB=pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeB[DynFrameCountArray_Index[i]];
			...
			case 3:
				...
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.nFrameNumber[DynFrameCountArray_Index[i]]==FrameNumber){					
					...
					MPEG4EConfigStructures->tForceframe.IntraRefreshVOP=pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.ForceIFrame[DynFrameCountArray_Index[i]];
			...
			case 4:
				...
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nFrameNumber[DynFrameCountArray_Index[i]]==FrameNumber){					
					...
					MPEG4EConfigStructures->tQPSettings.nQpI=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpI[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tQPSettings.nQpMaxI=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxI[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tQPSettings.nQpMinI=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinI[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tQPSettings.nQpP=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpP[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tQPSettings.nQpMaxP=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxP[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tQPSettings.nQpMinP=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinP[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tQPSettings.nQpOffsetB=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpOffsetB[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tQPSettings.nQpMaxB=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxB[DynFrameCountArray_Index[i]];
					MPEG4EConfigStructures->tQPSettings.nQpMinB=pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinB[DynFrameCountArray_Index[i]];
			...
			case 8:
				...
				if(pAppData->MPEG4_TestCaseParamsDynamic->DynPixelInfo.nFrameNumber[DynFrameCountArray_Index[i]]==FrameNumber){					
					...
					eError = OMX_GetConfig(pHandle, OMX_TI_IndexConfigVideoPixelInfo, &(MPEG4EConfigStructures->tPixelInfo));
					GOTO_EXIT_IF((eError != OMX_ErrorNone),eError);
	...
	if(fgets(line,254,fpconfigFile)){
		sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->TestCaseId = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the testcase_ID. Check the file syntax");
            goto EXIT;
         }
	if(fgets(line,254,fpconfigFile)){
		 ...
		 if (!pAppData->fIn)
		{
			MPEG4CLIENT_ERROR_PRINT( "%s - File not found. Place the file in the location of the base_image/input",file_string);
			return OMX_ErrorInsufficientResources;
		}
         }else{
         	eError = OMX_ErrorBadParameter;
             MPEG4CLIENT_ERROR_PRINT("Could not get the input file. Check the file syntax");
	        goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%s",file_string);
		  ...
		  pAppData->fOut= fopen(file_string, "wb");
		   if (!pAppData->fOut)
		{
			MPEG4CLIENT_ERROR_PRINT( "%s - File not found. Place the file in the location of the base_image/output",file_string);
			return OMX_ErrorInsufficientResources;
		}
        }else{
        	eError = OMX_ErrorBadParameter;
             MPEG4CLIENT_ERROR_PRINT("Could not get the output file. Check the file syntax");
	        goto EXIT;
         }
	  if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->width= file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the width. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->height= file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the height. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->profile = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the profile. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->level = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the level. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("chroma format = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->inputChromaFormat = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the inputchromaformat. Check the file syntax");
            goto EXIT;
         }
          if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("InputContentType = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->InputContentType = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the InputContentType. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("InterlaceCodingType = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->InterlaceCodingType = file_parameter;
         }else{
            MPEG4CLIENT_ERROR_PRINT("Could not get the InterlaceCodingType. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->bLoopFilter = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the bLoopFilter. Check the file syntax");
            goto EXIT;
         }
          if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->bCABAC = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the bCABAC. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->bFMO = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the bFMO. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("bConstIntraPred = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->bConstIntraPred = file_parameter;
         }else{
            MPEG4CLIENT_ERROR_PRINT("Could not get the bConstIntraPred. Check the file syntax");
            goto EXIT;
         }
          if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("nNumSliceGroups = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->nNumSliceGroups = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the nNumSliceGroups. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("nSliceGroupMapType = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->nSliceGroupMapType = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the nSliceGroupMapType. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->eSliceMode = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the eSliceMode. Check the file syntax");
            goto EXIT;
         }
          if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("RateControlAlg = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->RateControlAlg = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the RateControlAlg. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("TransformBlockSize = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->TransformBlockSize = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the TransformBlockSize. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("EncodingPreset = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->EncodingPreset = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the EncodingPreset. Check the file syntax");
            goto EXIT;
         }
          if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("RateCntrlPreset = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->RateCntrlPreset = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the RateCntrlPreset. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("BitStreamFormat = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->BitStreamFormat = file_parameter;
         }else{
            MPEG4CLIENT_ERROR_PRINT("Could not get the BitStreamFormat. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("maxInterFrameInterval = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->maxInterFrameInterval = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the maxInterFrameInterval. Check the file syntax");
            goto EXIT;
         }
         pAppData->MPEG4_TestCaseParams->nBitEnableAdvanced=0;
          if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("nBitEnableAdvanced = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->nBitEnableAdvanced = file_parameter;
         }else{
            MPEG4CLIENT_ERROR_PRINT("Could not get the nBitEnableAdvanced. Check the file syntax");
            goto EXIT;
         }
         pAppData->MPEG4_TestCaseParams->nBitEnableDynamic=0;
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("nBitEnableDynamic = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->nBitEnableDynamic = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the nBitEnableDynamic. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
            ...
            pAppData->MPEG4_TestCaseParams->nNumInputBuf = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the nNumInputBuf. Check the file syntax");
            goto EXIT;
         }
          if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("nNumOutputBuf = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->nNumOutputBuf = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the nNumOutputBuf. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("bInAllocatebuffer = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->bInAllocatebuffer = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the bInAllocatebuffer. Check the file syntax");
            goto EXIT;
         }
         if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("bOutAllocatebuffer = %d", file_parameter);
            pAppData->MPEG4_TestCaseParams->eSliceMode = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the eSliceMode. Check the file syntax");
         ...
         if(fgets(line,254,fpconfigFile)){
            ...
            pAppData->MPEG4_TestCaseParams->TestType = file_parameter;
         	...
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the TestType. Check the file syntax");
            goto EXIT;
         }
        if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
...
if(pAppData->MPEG4_TestCaseParams->nBitEnableDynamic){
	sprintf(EncConfigFile,"%s_Dynamic_TC_%d.cfg",configFile,testcasenum);
	MPEG4CLIENT_INFO_PRINT("open file Encoder dynamic Settings.cfg ");
	fpconfigFile = fopen(EncConfigFile,"r");
	if (!fpconfigFile)
	{
		MPEG4CLIENT_ERROR_PRINT( "%s - File not found. Place the file in the location of the base_image",EncConfigFile);
	...
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynFrameRate.nFrameNumber[i]=MaxParam[i];
	}
	...
	/*get the frame numbers in the structure*/
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynBitRate.nFrameNumber[i]=MaxParam[i];
	}
	/*update for the Free Index count*/
	DynFrameCountArray_FreeIndex[1]=NumParamCount;
	if(NumParamCount){
		 pAppData->MPEG4_TestCaseParamsDynamic->DynBitRate.nBitrate=(OMX_U32*)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynBitRate.nBitrate== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
	...
	/*get the frame numbers in the structure*/
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nFrameNumber[i]=MaxParam[i];
	}
	/*update for the Free Index count*/
	DynFrameCountArray_FreeIndex[2]=NumParamCount;
	 if(NumParamCount){
		 pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nMVAccuracy=(OMX_U32*)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nMVAccuracy== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		 pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeP=(OMX_U32*)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeP== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		 pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeP=(OMX_U32*)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeP== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		 pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeB=(OMX_U32*)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
	...
	/*get the frame numbers in the structure*/
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.nFrameNumber[i]=MaxParam[i];
	}
	/*update for the Free Index count*/
	DynFrameCountArray_FreeIndex[3]=NumParamCount;
	if(NumParamCount){
		 pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.ForceIFrame=(OMX_BOOL *)TIMM_OSAL_Malloc((sizeof(OMX_BOOL )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.ForceIFrame== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		if(fgets(line,254,fpconfigFile)){
			j=sscanf(line,"%d %d %d %d %d %d %d %d %d %d",&MaxParam[0],&MaxParam[1],&MaxParam[2],&MaxParam[3],&MaxParam[4],&MaxParam[5],&MaxParam[6],&MaxParam[7],&MaxParam[8],&MaxParam[9]);
			for(i=0;i<NumParamCount;i++){
				MPEG4CLIENT_INFO_PRINT("Param = %d",MaxParam[i]);
	...
	/*get the frame numbers in the structure*/
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nFrameNumber[i]=MaxParam[i];
	}
	/*update for the Free Index count*/
	DynFrameCountArray_FreeIndex[4]=NumParamCount;
	if(NumParamCount){
		//QpI
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpI=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpI== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		//QpIMax
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxI=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxI== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		//QpIMin
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinI=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinI== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		//QpP
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpP=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpP== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		//QpPMax
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxP=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxP== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		//QpPMin
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinP=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinP== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		//QpB
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpOffsetB=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpOffsetB== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		//QpBMax
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxB=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxB== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		//QpBMin
		 pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinB=(OMX_U32 *)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
		GOTO_EXIT_IF((pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinB== TIMM_OSAL_NULL),OMX_ErrorInsufficientResources);
		if(fgets(line,254,fpconfigFile)){
			j=sscanf(line,"%d %d %d %d %d %d %d %d %d %d",&MaxParam[0],&MaxParam[1],&MaxParam[2],&MaxParam[3],&MaxParam[4],&MaxParam[5],&MaxParam[6],&MaxParam[7],&MaxParam[8],&MaxParam[9]);
			for(i=0;i<NumParamCount;i++){
				MPEG4CLIENT_INFO_PRINT("Param = %d",MaxParam[i]);
	...
	/*get the frame numbers in the structure*/
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynIntraFrmaeInterval.nFrameNumber[i]=MaxParam[i];
	}
	/*update for the Free Index count*/
	...
	/*get the frame numbers in the structure*/
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynNALSize.nFrameNumber[i]=MaxParam[i];
	}
	/*update for the Free Index count*/
	...
	/*get the frame numbers in the structure*/
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynSliceSettings.nFrameNumber[i]=MaxParam[i];
	}
	/*update for the Free Index count*/
	DynFrameCountArray_FreeIndex[7]=NumParamCount;
	if(NumParamCount){
		 pAppData->MPEG4_TestCaseParamsDynamic->DynSliceSettings.eSliceMode=(OMX_VIDEO_AVCSLICEMODETYPE*)TIMM_OSAL_Malloc((sizeof(OMX_VIDEO_AVCSLICEMODETYPE )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
	...
	/*get the frame numbers in the structure*/
	for(i=0;i<NumParamCount+1;i++){
		pAppData->MPEG4_TestCaseParamsDynamic->DynPixelInfo.nFrameNumber[i]=MaxParam[i];
	}
	/*update for the Free Index count*/
	DynFrameCountArray_FreeIndex[8]=NumParamCount;
	if(NumParamCount){
		 pAppData->MPEG4_TestCaseParamsDynamic->DynPixelInfo.nWidth=(OMX_U32*)TIMM_OSAL_Malloc((sizeof(OMX_U32 )* NumParamCount), TIMM_OSAL_TRUE, 0 ,TIMMOSAL_MEM_SEGMENT_EXT);
...
if(pAppData->MPEG4_TestCaseParams->nBitEnableAdvanced){
	sprintf(EncConfigFile,"%s_Advanced_TC_%d.cfg",configFile,testcasenum);
	MPEG4CLIENT_INFO_PRINT("open file EncoderAdvanced Settings.cfg ");
	fpconfigFile = fopen(EncConfigFile,"r");
	if (!fpconfigFile)
	{
		MPEG4CLIENT_ERROR_PRINT( "%s - File not found. Place the file in the location of the base_image",EncConfigFile);
	...
	if(fgets(line,254,fpconfigFile)){
    	    ...
    	    MPEG4CLIENT_INFO_PRINT("NALU.nStartofSequence = %x", file_parameter);
            pAppData->MPEG4_TestCaseParamsAdvanced->NALU.nStartofSequence = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the NALU.nStartofSequence. Check the file syntax");
            goto EXIT;
         }
	//NALU.nEndofSequence
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%x",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("NALU.nEndofSequence = %x", file_parameter);
            pAppData->MPEG4_TestCaseParamsAdvanced->NALU.nEndofSequence = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the NALU.nEndofSequence. Check the file syntax");
            goto EXIT;
         }
	//NALU.nIDR
 	if(fgets(line,254,fpconfigFile)){
		 sscanf(line,"%x",&file_parameter);
		 MPEG4CLIENT_INFO_PRINT("NALU.nIDR = %x", file_parameter);
		 pAppData->MPEG4_TestCaseParamsAdvanced->NALU.nIDR = file_parameter;
	}else{
		eError = OMX_ErrorBadParameter;
		MPEG4CLIENT_ERROR_PRINT("Could not get the NALU.nIDR. Check the file syntax");
		goto EXIT;
	}
	//NALU.nIntraPicture
 	if(fgets(line,254,fpconfigFile)){
		 sscanf(line,"%x",&file_parameter);
		 MPEG4CLIENT_INFO_PRINT("NALU.nIntraPicture = %x", file_parameter);
		 pAppData->MPEG4_TestCaseParamsAdvanced->NALU.nIntraPicture = file_parameter;
	}else{
		eError = OMX_ErrorBadParameter;
		MPEG4CLIENT_ERROR_PRINT("Could not get the NALU.nIntraPicture. Check the file syntax");
		goto EXIT;
	}
	//NALU.nNonIntraPicture
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%x",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("NALU.nNonIntraPicture = %x", file_parameter);
            pAppData->MPEG4_TestCaseParamsAdvanced->NALU.nNonIntraPicture = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the NALU.nNonIntraPicture. Check the file syntax");
            goto EXIT;
         }
	...
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->MEBlock.eMinBlockSizeP = %d", file_parameter);
            pAppData->MPEG4_TestCaseParamsAdvanced->MEBlock.eMinBlockSizeP = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the pAppData->MPEG4_TestCaseParamsAdvanced->MEBlock.eMinBlockSizeP Check the file syntax");
            goto EXIT;
        }
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->MEBlock.eMinBlockSizeB = %d", file_parameter);
            pAppData->MPEG4_TestCaseParamsAdvanced->MEBlock.eMinBlockSizeB = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the pAppData->MPEG4_TestCaseParamsAdvanced->MEBlock.eMinBlockSizeB Check the file syntax");
            goto EXIT;
        }
	//IntraRefreshSettings IntraRefresh;
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->IntraRefresh.eRefreshMode = %d", file_parameter);
            pAppData->MPEG4_TestCaseParamsAdvanced->IntraRefresh.eRefreshMode = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the pAppData->MPEG4_TestCaseParamsAdvanced->IntraRefresh.eRefreshMode Check the file syntax");
            goto EXIT;
        }
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->IntraRefresh.nRate= %d", file_parameter);
          pAppData->MPEG4_TestCaseParamsAdvanced->IntraRefresh.nRate = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the pAppData->MPEG4_TestCaseParamsAdvanced->IntraRefresh.nRate Check the file syntax");
            goto EXIT;
        }
	//VUISettings VUI;
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->VUI.bAspectRatioPresent= %d", file_parameter);
          pAppData->MPEG4_TestCaseParamsAdvanced->VUI.bAspectRatioPresent = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the pAppData->MPEG4_TestCaseParamsAdvanced->VUI.bAspectRatioPresent Check the file syntax");
            goto EXIT;
        }
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->VUI.eAspectRatio= %d", file_parameter);
          pAppData->MPEG4_TestCaseParamsAdvanced->VUI.eAspectRatio = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
            MPEG4CLIENT_ERROR_PRINT("Could not get the pAppData->MPEG4_TestCaseParamsAdvanced->VUI.eAspectRatio Check the file syntax");
            goto EXIT;
        }
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->VUI.bFullRange= %d", file_parameter);
          pAppData->MPEG4_TestCaseParamsAdvanced->VUI.bFullRange = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%x",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->IntraPred.nLumaIntra4x4Enable= %x", file_parameter);
          pAppData->MPEG4_TestCaseParamsAdvanced->IntraPred.nLumaIntra4x4Enable = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
         ...
         pAppData->MPEG4_TestCaseParamsAdvanced->IntraPred.nLumaIntra8x8Enable = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
          ...
          pAppData->MPEG4_TestCaseParamsAdvanced->IntraPred.nLumaIntra16x16Enable = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
          ...
          pAppData->MPEG4_TestCaseParamsAdvanced->IntraPred.nChromaIntra8x8Enable = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
          ...
          pAppData->MPEG4_TestCaseParamsAdvanced->IntraPred.eChromaComponentEnable = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
            sscanf(line,"%d",&file_parameter);
    	    MPEG4CLIENT_INFO_PRINT("pAppData->MPEG4_TestCaseParamsAdvanced->DataSync.inputDataMode= %d", file_parameter);
          pAppData->MPEG4_TestCaseParamsAdvanced->DataSync.inputDataMode= file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
          ...
          pAppData->MPEG4_TestCaseParamsAdvanced->DataSync.outputDataMode = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
          ...
          pAppData->MPEG4_TestCaseParamsAdvanced->DataSync.numInputDataUnits = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
	...
	if(fgets(line,254,fpconfigFile)){
          ...
          pAppData->MPEG4_TestCaseParamsAdvanced->DataSync.numOutputDataUnits = file_parameter;
         }else{
         	eError = OMX_ErrorBadParameter;
...
OMX_ERRORTYPE MPEG4E_FreeDynamicClientParams(MPEG4E_ILClient* pApplicationData){
	...
	if(!pApplicationData){
		...
		for(i=0;i<9;i++){
			switch(i){
				case 0:
					TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynFrameRate.nFrameNumber);
					if(DynFrameCountArray_FreeIndex[i]){
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynFrameRate.nFramerate){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynFrameRate.nFramerate);
						}else{
				...
				case 2:
					...
					if(DynFrameCountArray_FreeIndex[i]){
						...
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeP){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeP);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeP){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeP);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeB){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nHorSearchRangeB);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeB){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynMESearchRange.nVerSearchRangeB);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
				...
				break;
				case 3:
					TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.nFrameNumber);
					if(DynFrameCountArray_FreeIndex[i]){
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.ForceIFrame){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynForceFrame.ForceIFrame);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
				...
				break;
				case 4:
					TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nFrameNumber);
					if(DynFrameCountArray_FreeIndex[i]){
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpI){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpI);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxI){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxI);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinI){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinI);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpP){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpP);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxP){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxP);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinP){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinP);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpOffsetB){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpOffsetB);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxB){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMaxB);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinB){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynQpSettings.nQpMinB);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}						
				...
				break;
				case 5:
					TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynIntraFrmaeInterval.nFrameNumber);
					if(DynFrameCountArray_FreeIndex[i]){
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynIntraFrmaeInterval.nIntraFrameInterval){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynIntraFrmaeInterval.nIntraFrameInterval);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}			
				...
				break;
				case 6:
					TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynNALSize.nFrameNumber);
					if(DynFrameCountArray_FreeIndex[i]){
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynNALSize.nNaluSize){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynNALSize.nNaluSize);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
				...
				break;
				case 7:
					TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynSliceSettings.nFrameNumber);
					if(DynFrameCountArray_FreeIndex[i]){
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynSliceSettings.eSliceMode){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynSliceSettings.eSliceMode);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynSliceSettings.nSlicesize){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynSliceSettings.nSlicesize);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
				...
				case 8:
					...
					if(DynFrameCountArray_FreeIndex[i]){					
						...
						if(pAppData->MPEG4_TestCaseParamsDynamic->DynPixelInfo.nHeight){
							TIMM_OSAL_Free(pAppData->MPEG4_TestCaseParamsDynamic->DynPixelInfo.nHeight);
						}else{
							eError=OMX_ErrorUndefined;
							MPEG4CLIENT_ERROR_PRINT( "Something has gone Wrong..........................!!!!!!!!!!!!!");
						}
